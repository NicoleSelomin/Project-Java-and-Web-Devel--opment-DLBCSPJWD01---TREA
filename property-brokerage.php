...
/*
|--------------------------------------------------------------------------
| property-brokerage.php
|--------------------------------------------------------------------------
| Lists all available properties under brokerage service with dynamic filter and pagination.
...
$countStmt = $pdo->prepare("
    ...
    WHERE s.slug = 'brokerage' AND p.availability = 'available' 
    $whereSql
");
...
<head>
    ...
    <title>Brokerage Properties | TREA</title>
...
<main class="container py-5 flex-grow-1">
    ...
    <div class="mb-4 p-3 border rounded shadow-sm text-center main-title bg-white">
        <h2 class="mb-2">Available Properties for Brokerage: Buy or Rent</h2>
        <p class="text-muted mb-0 fs-6">All listed properties are available via our brokerage service. Connect directly with owners for your next home.</p>
    </div>
    ...
    <div class="alert alert-info small d-flex align-items-center mb-4" role="alert" style="max-width:720px; margin:auto;">
        ...
        <div>
            <strong>What does <span class="badge bg-secondary">Brokerage</span> mean?</strong>
            TREA connects you to the property owner but does not manage contracts or processes after introduction. For fully managed properties, look for the <span class="badge bg-success">Managed</span> badge.
        </div>
    </div>    

    <!-- Property Cards Grid -->
    <?php if (empty($properties)): ?>
        <div class="alert alert-warning text-center">No brokerage properties found. Try adjusting your filters or check back soon!</div>
    <?php else: ?>
    <div class="row row-cols-1 row-cols-md-3 g-4">
        <?php foreach ($properties as $property): ?>
            <div class="col">
                <div class="card h-100 shadow-sm<?= $property['urgent'] ? ' border-danger border-2' : '' ?>">
                    <!-- Property Image -->
                    ...
                    <div class="card-body d-flex flex-column">
                        <a href="view-property.php?property_id=<?= $property['property_id'] ?>" class="text-decoration-none text-dark">
                            <h5 class="card-title mb-1">
                                ...
                                <span class="badge bg-secondary ms-1" data-bs-toggle="tooltip" title="Brokerage: TREA only connects you to the owner.">Brokerage</span>
                            </h5>
                        </a>
